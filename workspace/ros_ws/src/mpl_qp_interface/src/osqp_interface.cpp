...
#include "mpl_qp_interface/osqp_csc_matrix_conv.hpp"
...
namespace mpl::qp_interface
{
OSQPInterface::OSQPInterface(const bool enable_warm_start, const int max_iteration,
    const c_float eps_abs, const c_float eps_rel, const bool polish, const bool verbose)
...
OSQPInterface::OSQPInterface(const Eigen::MatrixXd & P, const Eigen::MatrixXd & A,
    ...
    const bool enable_warm_start, const c_float eps_abs)
    : OSQPInterface(enable_warm_start, OSQP_MAX_ITERATION, eps_abs)
{
	initializeProblem(P, A, q, l, u);
}

OSQPInterface::OSQPInterface(const CSC_Matrix & P, const CSC_Matrix & A,
    const std::vector<double> & q, const std::vector<double> & l, const std::vector<double> & u,
    const bool enable_warm_start, const c_float eps_abs)
    : OSQPInterface(enable_warm_start, OSQP_MAX_ITERATION, eps_abs)
{
...
void OSQPInterface::initializeProblemImpl(const Eigen::MatrixXd & P, const Eigen::MatrixXd & A,
    const std::vector<double> & q, const std::vector<double> & l, const std::vector<double> & u)
{
...
void OSQPInterface::initializeCSCProblemImpl(CSC_Matrix P_csc, CSC_Matrix A_csc,
    const std::vector<double> & q, const std::vector<double> & l, const std::vector<double> & u)
{
	...
	data_->n = param_n_;
	if (data_->P) free(data_->P);
	data_->P = csc_matrix(data_->n, data_->n, static_cast<c_int>(P_csc.vals_.size()),
	    P_csc.vals_.data(), P_csc.row_idxs_.data(), P_csc.col_idxs_.data());
	data_->q = q_dyn;
	if (data_->A) free(data_->A);
	data_->A = csc_matrix(data_->m, data_->n, static_cast<c_int>(A_csc.vals_.size()),
	    A_csc.vals_.data(), A_csc.row_idxs_.data(), A_csc.col_idxs_.data());
	data_->l = l_dyn;
	data_->u = u_dyn;
...
void OSQPInterface::updateP(const Eigen::MatrixXd & P_new)
{
	...
	*/
	...
	osqp_update_P(
	    work_.get(), P_csc.vals_.data(), OSQP_NULL, static_cast<c_int>(P_csc.vals_.size()));
}
...
void OSQPInterface::updateA(const Eigen::MatrixXd & A_new)
{
	...
	*/
	...
	osqp_update_A(
	    work_.get(), A_csc.vals_.data(), OSQP_NULL, static_cast<c_int>(A_csc.vals_.size()));
	return;
}
...
std::vector<double> OSQPInterface::optimize(CSC_Matrix P, CSC_Matrix A,
    const std::vector<double> & q, const std::vector<double> & l, const std::vector<double> & u)
{
	initializeCSCProblemImpl(P, A, q, l, u);
