...
{
	using mpl::qp_interface::calCSCMatrix;
	using mpl::qp_interface::calCSCMatrixTrapezoidal;
	using mpl::qp_interface::CSC_Matrix;

	auto check_result = [](const auto & solution, const std::string & status,
	                        const int polish_status) {
	...
	const std::vector<double> l = {1.0, 0.0, 0.0, -mpl::qp_interface::OSQP_INF};
	const std::vector<double> u = {1.0, 0.7, 0.7, mpl::qp_interface::OSQP_INF};

	{
		// Define problem during optimization
		mpl::qp_interface::OSQPInterface osqp(false, 4000, 1e-6);
	...
	// add warm startup
	{
		...
		std::vector<double> l_ini(4, 0.0);
		std::vector<double> u_ini(4, 0.0);
		mpl::qp_interface::OSQPInterface osqp(true, 4000, 1e-6);  // enable warm start
		osqp.optimize(P_ini_csc, A_ini_csc, q_ini, l_ini, u_ini);
