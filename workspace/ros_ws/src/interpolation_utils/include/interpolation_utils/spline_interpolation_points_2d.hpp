...
public:
  ...
  explicit SplineInterpolationPoints2d(const std::vector<T> & points)
  {
    ...
    for (const auto & p : points) {
      points_inner.push_back(mpl::geometry_utils::get_point(p));
    }
    calcSplineCoefficientsInner(points_inner);
  }

  template <typename T> 
  inline void initPoints(const std::vector<T> & points, mpl::rclcpp_utils::Logger& logger)
  {
    //logger.info("in intiPoints");
    std::vector<geometry_msgs::msg::Point> points_inner;
    
    for (const auto & p : points) {
      auto point = mpl::geometry_utils::get_point(p);
      points_inner.push_back(point);
    }
    //logger.info("calling calcSplineCoefficientsInner");
    calcSplineCoefficientsInner(points_inner);
  }
  ...
  std::vector<geometry_msgs::msg::Pose> getSplineInterpolatedPoses(
