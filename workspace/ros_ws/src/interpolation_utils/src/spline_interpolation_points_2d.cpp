    ...
    const std::vector<geometry_msgs::msg::Point> & points)
{
	...
	for (size_t i = 0; i < points.size(); i++) {
		...
		if (i > 0) {
			...
			if (std::fabs(current_pos.x - prev_pos.x) < 1e-6 &&
...
std::vector<double> splineYawFromPoints(const std::vector<T> & points)
{
	...
	for (size_t i = 0; i < points.size(); ++i) {
		const auto distance =
		    mpl::geometry_utils::calcDistance2d(points[i], interpolated_points[interpolated_idx]);
...
template std::vector<double> splineYawFromPoints(
    const std::vector<project_utils_msgs::msg::PathPoint> & points);
// template std::vector<double> splineYawFromPoints(
    ...
    const size_t idx, const double s) const
{
	...
	pose.orientation =
	    mpl::geometry_utils::createQuaternionFromYaw(getSplineInterpolatedYaw(idx, s));
	return pose;
}

std::vector<geometry_msgs::msg::Pose> SplineInterpolationPoints2d::getSplineInterpolatedPoses(
    const std::vector<double> & resampled_arclength) const
{
	std::vector<geometry_msgs::msg::Pose> poses;
	poses.reserve(resampled_arclength.size());

	for (const double s : resampled_arclength) {
		geometry_msgs::msg::Pose pose;

		// position from spline
		pose.position = getSplineInterpolatedPoint(0, s);

		// orientation from spline yaw
		pose.orientation =
		    mpl::geometry_utils::createQuaternionFromYaw(getSplineInterpolatedYaw(0, s));
