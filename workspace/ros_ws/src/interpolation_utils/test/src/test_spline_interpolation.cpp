...
using mpl::interpolation::SplineInterpolation;
...
TEST(spline_interpolation, spline)
{
  {  // straight: query_keys is same as base_keys
    ...
    const auto query_values = mpl::interpolation::spline(base_keys, base_values, query_keys);
...
TEST(spline_interpolation, splineByAkima)
{
  {  // straight: query_keys is same as base_keys
    ...
    const auto query_values = mpl::interpolation::splineByAkima(base_keys, base_values, query_keys);
    for (size_t i = 0; i < query_values.size(); ++i) {
      EXPECT_NEAR(query_values.at(i), ans.at(i), epsilon);
    }
  }
