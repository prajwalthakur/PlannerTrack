...
#include "interpolation_utils/linear_interpolation.hpp"
...
{
  EXPECT_EQ(mpl::interpolation::lerp(0.0, 1.0, 0.3), 0.3);
  EXPECT_EQ(mpl::interpolation::lerp(-0.5, 12.3, 0.3), 3.34);
}
...
TEST(linear_interpolation, lerp_vector)
{
  {  // straight: query_keys is same as base_keys
    ...
    const auto query_values = mpl::interpolation::lerp(base_keys, base_values, query_keys);
...
TEST(linear_interpolation, lerp_scalar_query)
{
  {  // curve: query_keys is same as random
    ...
    for (size_t i = 0; i < query_keys.size(); ++i) {
      const auto query_value = mpl::interpolation::lerp(base_keys, base_values, query_keys.at(i));
