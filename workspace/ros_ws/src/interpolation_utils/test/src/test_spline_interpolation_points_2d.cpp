...
#include "interpolation_utils/spline_interpolation.hpp"
...
using mpl::interpolation::SplineInterpolationPoints2d;

TEST(spline_interpolation, splineYawFromPoints)
{
	using mpl::geometry_utils::create_point;
	...
	{  // straight
		...
		const auto yaws = mpl::interpolation::splineYawFromPoints(points);
	...
	{  // size of base_keys is 1 (infeasible to interpolate)
		...
		EXPECT_THROW(mpl::interpolation::splineYawFromPoints(points), std::logic_error);
	}
...
TEST(spline_interpolation, CurvatureMatchesCircle)
{
  using mpl::geometry_utils::create_point;

  const double R = 5.0;
  const double expected_k = 1.0 / R;

  std::vector<geometry_msgs::msg::Point> points;

  // Dense arc (important!)
  for (double theta = -M_PI/2; theta <= 0.0; theta += M_PI/18) {
    points.push_back(create_point(R * cos(theta), R * sin(theta), 0.0));
  }
  ...
  for (int i = 0; i < (int)points.size() - 1; ++i) {
    for (double t = 0.0; t <= 1.0; t += 0.01) {
      curvatures.push_back(s.getSplineInterpolatedCurvature(i, t));
    }
  }

  // 1. Mean curvature ≈ 1/R
  double mean_k = 0.0;
  for (double k : curvatures) mean_k += k;
  mean_k /= curvatures.size();

  EXPECT_NEAR(mean_k, expected_k, 0.05);

  // 2. Variance should be small (smoothness)
  double var = 0.0;
