...
#include "interpolation_utils/interpolation_utils.hpp"
...
TEST(interpolation_utils, isIncreasing)
{
  ...
  EXPECT_THROW(mpl::interpolation::isIncreasing(empty_vec), std::invalid_argument);

  // increase
  const std::vector<double> increasing_vec{0.0, 1.5, 3.0, 4.5, 6.0};
  EXPECT_EQ(mpl::interpolation::isIncreasing(increasing_vec), true);

  // not decrease
  const std::vector<double> not_increasing_vec{0.0, 1.5, 1.5, 4.5, 6.0};
  EXPECT_EQ(mpl::interpolation::isIncreasing(not_increasing_vec), false);

  // decrease
  const std::vector<double> decreasing_vec{0.0, 1.5, 1.2, 4.5, 6.0};
  EXPECT_EQ(mpl::interpolation::isIncreasing(decreasing_vec), false);
}
...
TEST(interpolation_utils, isNotDecreasing)
{
  // empty
  const std::vector<double> empty_vec;
  EXPECT_THROW(mpl::interpolation::isNotDecreasing(empty_vec), std::invalid_argument);

  // increase
  const std::vector<double> increasing_vec{0.0, 1.5, 3.0, 4.5, 6.0};
  EXPECT_EQ(mpl::interpolation::isNotDecreasing(increasing_vec), true);

  // not decrease
  const std::vector<double> not_increasing_vec{0.0, 1.5, 1.5, 4.5, 6.0};
  EXPECT_EQ(mpl::interpolation::isNotDecreasing(not_increasing_vec), true);

  // decrease
  const std::vector<double> decreasing_vec{0.0, 1.5, 1.2, 4.5, 6.0};
  EXPECT_EQ(mpl::interpolation::isNotDecreasing(decreasing_vec), false);
}

TEST(interpolation_utils, validateKeys)
{
  using mpl::interpolation::validateKeys;
...
{
  using mpl::interpolation::validateKeysAndValues;
