...
#include "motion_model_shapes/collision_model/ellipse_collision_footprint.hpp"
...
EllipseCollisionFootPrint::EllipseCollisionFootPrint(
    const double majorAxisLength, const double minorAxisLength)
...
void EllipseCollisionFootPrint::initialize(const YAML::Node & params)
{
	mMajorAxisLength = params["major_axis_length"].as<double>();
	mMinorAxisLength = params["minor_axis_length"].as<double>();
	mSemiMajorAxisLength = mMajorAxisLength / 2.0;
	mSemiMinorAxisLength = mMinorAxisLength / 2.0;
	mPose = std::make_shared<stPose>();
}

//////////////////////////////////////////////////////////////////////////

void EllipseCollisionFootPrint::step(const stPose & pose)
{
...
{
	Eigen::Matrix2f D;
	D << 1.0f / (mSemiMajorAxisLength * mSemiMajorAxisLength), 0, 0,
	    1.0f / (mSemiMinorAxisLength * mSemiMinorAxisLength);
...
std::pair<double, bool> EllipseCollisionFootPrint::detectCollision(
    const std::shared_ptr<CollisionFootPrint> object1,
    const std::shared_ptr<CollisionFootPrint> object2)
{
	...
	auto k_function = [&](float s) {
		...
		Eigen::Matrix2f M = (1.0f / (1.0f - s)) * Ainv + (1.0f / s) * Binv;
