...
#include "mpl_osqp_interface/osqp_interface.hpp"
...
{
	using mpl::osqp_interface::calCSCMatrix;
	using mpl::osqp_interface::calCSCMatrixTrapezoidal;
	using mpl::osqp_interface::CSC_Matrix;

	auto check_result = [](const mpl::osqp_interface::OSQPResult & result) {
	...
	const std::vector<double> l = {1.0, 0.0, 0.0, -mpl::osqp_interface::INF};
	const std::vector<double> u = {1.0, 0.7, 0.7, mpl::osqp_interface::INF};

	{
		// Define problem during optimization
		mpl::osqp_interface::OSQPInterface osqp;
		mpl::osqp_interface::OSQPResult result = osqp.optimize(P, A, q, l, u);
		check_result(result);
	}

	{
		// Define problem during initialization
		mpl::osqp_interface::OSQPInterface osqp(P, A, q, l, u, 1e-6);
		mpl::osqp_interface::OSQPResult result = osqp.optimize();
		check_result(result);
	}

	{
		mpl::osqp_interface::OSQPResult result;
		...
		mpl::osqp_interface::OSQPInterface osqp(P_ini, A_ini, q_ini, l_ini, u_ini, 1e-6);
		osqp.optimize();
...

	{
		...
		mpl::osqp_interface::OSQPInterface osqp(P_csc, A_csc, q, l, u, 1e-6);
		mpl::osqp_interface::OSQPResult result = osqp.optimize();
		check_result(result);
	}

	{
		...
		std::vector<double> l_ini(4, 0.0);
		std::vector<double> u_ini(4, 0.0);
		mpl::osqp_interface::OSQPInterface osqp(P_ini_csc, A_ini_csc, q_ini, l_ini, u_ini, 1e-6);
		osqp.optimize();
...
{
	using mpl::osqp_interface::OSQPInterface;
