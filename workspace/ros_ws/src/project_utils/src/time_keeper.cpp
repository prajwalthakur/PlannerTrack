...
#include "project_utils/time_keeper.hpp"
...
std::string ProcessingTimeNode::to_string() const
{
	std::function<void(
	    ...
	    construct_string = [&](const ProcessingTimeNode & node, std::ostringstream & oss,
		    ...
		    for (size_t i = 0; i < node.child_nodes_.size(); ++i) {
			    ...
			    construct_string(*child, oss, prefix + (is_last ? "    " : "│   "),
			        i == node.child_nodes_.size() - 1, false);
		    }
	    };
...
project_utils_msgs::msg::ProcessingTimeTree ProcessingTimeNode::to_msg() const
{
	project_utils_msgs::msg::ProcessingTimeTree time_tree_msg;

	std::function<void(
	    const ProcessingTimeNode &, project_utils_msgs::msg::ProcessingTimeTree &, int)>
	    construct_msg = [&](const ProcessingTimeNode & node,
	                        project_utils_msgs::msg::ProcessingTimeTree & tree_msg, int parent_id) {
		    project_utils_msgs::msg::ProcessingTimeNode time_node_msg;
