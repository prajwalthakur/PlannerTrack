...
SingleTrackDynModel::SingleTrackDynModel(
    YAML::Node & simConfig, YAML::Node & vehConfig, long int value)
    : BaseType(simConfig, vehConfig)
{
	...
	if (isStringEqual(mVehConfig["geomModelType"].as<std::string>(), "rectangular")) {
		mGeomModel = std::make_shared<RectangularGeometry>(
		    mVehConfig["vehLength"].as<double>(), mVehConfig["vehWidth"].as<double>());
	}
	if (isStringEqual(mVehConfig["collFootPrintType"].as<std::string>(), "ellipse")) {
		...
		ptSharedPtr<CollisionFootPrint> collFootPrint =
		    std::make_shared<EllipseCollisionFootPrint>(majorAxis, minorAxis);
...
bool SingleTrackDynModel::checkNumStatesInputs(
    const StateVector & stateVec, const InputVector & inputVec)
{
	...
	if (stateVec.size() != mNumStates || inputVec.size() != mNumControlInputs) return false;
	return true;
}
