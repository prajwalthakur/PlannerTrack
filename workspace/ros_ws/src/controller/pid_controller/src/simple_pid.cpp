...
/**
 * Same as tick(), but the integral/derivative terms are computed from the
 * caller-supplied dt (seconds) directly, instead of registerTimeFunction()'s
 * long/unsigned-long clock -- see the declaration in simple_pid.hpp for why.
 */
template <class T>
T PIDController<T>::tick(double dt)
{
  currentFeedback = _pidSource();
  if(enabled)
  {
    ...
    //Integrate/differentiate against the real elapsed time instead of a
    //per-tick count, so ki/kd stay in per-second units regardless of rate.
    const double cycleIntegral = ((lastError + error) / 2.0) * dt;
    integralCumulation += cycleIntegral;
    cycleDerivative = (dt > 0.0) ? (error - lastError) / dt : 0;
