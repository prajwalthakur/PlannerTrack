...
void mppi_critic::PathAlignCritic::score(CriticData& data)
{
    ...
    mLogger.info("pathSegmentsCount %ld", pathSegmentsCount);
    ...
    std::vector<bool>& pathPtsValid = data.path_pts_valid.value();
    ...
    size_t trajSampledSize = T_x.cols();
    ...
    //mLogger.info("path_integrated_distances size %ld, last el : %.3f", path_integrated_distances.size(), path_integrated_distances.at(path_integrated_distances.size()-1) );
    for(size_t batchIter = 0 ; batchIter < batchSize ; ++ batchIter)
    {
        ...
        pathPt = 0u;
        for(size_t j=1; j<trajSampledSize; ++j )
        {
            ...
            if(pathPtsValid[pathPt])
            {
                ...
                {
                    //mLogger.info("batch num %ld,  traj step num %ld , pose-yaw %.3f traj-yaw %.3f ", batchIter, j , pose.theta, T_yaw(batchIter, j));
                    summedPathDist+=sqrtf(dx * dx + dy * dy);
                }
            }
        }
        if(numSamples>0u)
