 ...
 */
#include "motion_model_ground_vehicles/bicycleKinematicModel/BicycleKinematicModel.hpp"
    ...
    YAML::Node & simConfig, YAML::Node & vehConfig, const UniqueId & id)
{
	...
	mSterringTau = mVehConfig["steeringTimeConstant"].as<double>();
}

//////////////////////////////////////////////////////////////////////////

void BicycleKinematicModel::createIntegrator()
{
...
void BicycleKinematicModel::createIntegrator(YAML::Node & simConfig, YAML::Node & vehYamlConfig)
{
...
void BicycleKinematicModel::reset()
{
...
void BicycleKinematicModel::setState(const StateVector & statevector)
{
...
void BicycleKinematicModel::setInput(const InputVector & input_vector)
{
...
const StateVector & BicycleKinematicModel::getState() const
{
	return mStateVector;
}
...
const StateVector & BicycleKinematicModel::getInput() const
{
	return mInputVector;
}
...
StateVector BicycleKinematicModel::xdot(
...
void BicycleKinematicModel::updateCommandedControl()
{
...
void BicycleKinematicModel::updateCommandedControl(const InputVector & u)
{
	mCommandedControl = u;
}
...
void BicycleKinematicModel::step()
{
...
StateVector BicycleKinematicModel::StateToVector(const StateStruct & state_struct) const
{
...
InputVector BicycleKinematicModel::InputToVector(const InputStruct & input_struct) const
{
...
InputVector BicycleKinematicModel::packAccelSteerRate(double acc, double steeringRate) const
{
...
StateStruct BicycleKinematicModel::VectorToState(const StateVector & statevector) const
{
	StateStruct st;
	st.x = statevector(0);
	st.y = statevector(1);
...
stPose BicycleKinematicModel::getStatePose() const
{
	stPose st =
...
InputStruct BicycleKinematicModel::VectorToInput(const InputVector & inputvector) const
{
	InputStruct input;
...
void BicycleKinematicModel::setupRos(
...
void BicycleKinematicModel::publishStates() const
{
	if (!mOdomPub) {
		return;
	}
...
PLUGINLIB_EXPORT_CLASS(BicycleKinematicModel, DynamicModel)
